...
public class QueryStringBuilderImpl<E> implements QueryStringBuilder<E>, Serializable {
    ...
    private transient static final Logger LOG = Logger.getLogger(QueryStringBuilderImpl.class.getName());
    ...
    public QueryStringBuilderImpl(PersistenceUnitContext jpaContext, Class<E> entityClass) {
    ...
    public void updateQuery(EntityManager em, Query q, Map whereClauseParameters, boolean convertCrossReferences) {
        ...
        LOG.log(Level.FINER, "Updating Query with: {0}", whereClauseParameters);        
        ...
        if(convertCrossReferences && !whereClauseParameters.isEmpty()) {
            joinColumns = this.jpaContext.getEntityReference().getJoinColumns(entityClass);
        }else{
            joinColumns = null;
        }
        ...
        for(Object key:whereClauseParameters.keySet()) {
            ...
            if(convertPossible) {
                ...
                Object ref = entityReference.getReferenceOptional(this.entityClass, joinColumns, col, val).orElse(null);
    ...
    public void updateQuery(EntityManager em, Query q, String col, 
        ...
        if(LOG.isLoggable(Level.FINER)) {
            LOG.log(Level.FINER, "Updating column: {0} with {1}", 
            new Object[]{col, (values==null?null:Arrays.toString(values))});                
        }    
        ...
        //
        final EntityReference entityReference = this.jpaContext.getEntityReference();
        ...
        for(int i=0; i<values.length; i++) {
            ...
            if(LOG.isLoggable(Level.FINER)) {
                LOG.log(Level.FINER, "At {0} setting {1} to {2}", new Object[]{i+1, col, val});            
            }
    ...
    public EntityManagerFactory getEntityManagerFactory() {
        return jpaContext.getEntityManagerFactory();
    }
    ...
    public String getSelectQuery(
        ...
        LOG.log(Level.FINE, "Query: {0}", selectQueryBuff);
    ...
    public final PersistenceUnitMetaData getMetaData() {
