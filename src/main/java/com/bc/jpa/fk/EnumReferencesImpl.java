...
public class EnumReferencesImpl<E> implements Serializable, EnumReferences<E> {
    ...
    public void validate() {
        ...
        if(logger.isLoggable(level)) {
            logger.log(level, "#validate(), enum types: {0}", enumTypes == null ? null : Arrays.toString(enumTypes));
        }
    }
    ...
    public Class getEnumType(String column) {
        ...
        for(Class enumType:enumTypes) {
            ...
            if(logger.isLoggable(Level.FINER)) {            
                logger.log(Level.FINER, "Enum type: {0}, referencing columns: {1}", 
                new Object[]{enumType, refingCols == null ? null : Arrays.toString(refingCols)});
            }
        ...
        if(logger.isLoggable(Level.FINER)) {
            logger.log(Level.FINER, "Referencing column: {0}, enum type: {1}", 
                    new Object[]{column, matchingType});
        }
        return matchingType;
    }
...
new Object[]{en.getDeclaringClass().getName(), en, dataColName, dataColValue});
    ...
    public void validate(Class aClass) {
        ...
        if(logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE, "Class: {0}, enum constants: {1}", 
                    new Object[]{aClass.getName(), 
                        (enumConstants==null?null:Arrays.toString(enumConstants))});
        }
