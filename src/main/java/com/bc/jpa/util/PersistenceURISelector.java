...
public class PersistenceURISelector {
    ...
    private transient static final Logger LOG = Logger.getLogger(PersistenceURISelector.class.getName());
    ...
    public URI selectURI(ClassLoader classLoader, String fname, URIFilter filter) throws IOException { 
        ...
        LOG.log(Level.FINE, "Selecting persistence files for name: {0}", fname);                
        ...
        if(uri == null) {
            ...
            if(file.exists()) {
                ...
                if(LOG.isLoggable(Level.INFO)) {
                    LOG.log(Level.INFO, "Created URI: {0}, from filename: {1}", new Object[]{uri, fname});
                }
            }else{
                LOG.log(Level.WARNING, "Does not exist file: {0}", fname);
            }
        }
        
        LOG.log(Level.INFO, "Selected persistence file: {0}", uri);                
        return uri;
    }
